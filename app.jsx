      ...
      >
        ...
        >
          <button
            style={{
              ...navStyles.navLink,
              ...(currentNav === "home" ? navStyles.navLinkActive : {}),
            }}
            onClick={() => setNav("home")}
          >
            Home
          ...
          <button
            style={{
              ...navStyles.navLink,
              ...(currentNav === "scan" ? navStyles.navLinkActive : {}),
            }}
            onClick={() => setNav("scan")}
          >
            Scan
          ...
          <button
            style={{
              ...navStyles.navLink,
              ...(currentNav === "tips" ? navStyles.navLinkActive : {}),
            }}
            onClick={() => setNav("tips")}
          >
            Tips
          ...
          <button
            style={{
              ...navStyles.navLink,
              ...(currentNav === "about" ? navStyles.navLinkActive : {}),
            }}
        ...
        >
          <button style={navStyles.loginBtn} onClick={() => alert("Login functionality coming soon!")}>
            Log In
          </button>
          <button style={navStyles.signupBtn} onClick={() => alert("Sign up functionality coming soon!")}>
            Sign Up
          </button>
        </div>
      </div>
...
function App() {
  ...
  const handleScan = async (e) => {
    ...
    try {
      ...
      // Mock result for demonstration if backend is not available
      if (url.includes("test-vuln.com")) {
        setResult([
          { type: "XSS", url: "/page?q=123", risk: "HIGH", description: "Reflected Cross-Site Scripting found in query parameter 'q'." },
          { type: "SQLi", url: "/login.php", risk: "CRITICAL", description: "Potential Blind SQL Injection vulnerability." }
        ]);
        setError(null);
      } else if (url.includes("test-ok.com")) {
         setResult([]);
         setError(null);
      }
      else {
        setError(err.message || "An error occurred. Check server logs if deployed.");
      }
    } finally {
      setLoading(false);
    }
  ...
  const downloadReport = () => {
    ...
    if (!result || result.length === 0) {
      ...
      html += result
        .map(
          (vuln) =>
            `<div style='margin-bottom:12px;padding:8px 12px;border-left:4px solid #d32f2f;background:#23171b;'><b>${vuln.type}</b> at <code>${vuln.url}</code>${
              vuln.risk ? ` <span style='color:#d32f2f'><b>[${vuln.risk}]</b></span>` : ""
            }${vuln.description ? `<div style='font-size:90%;margin-top:5px;'>${vuln.description}</div>` : ""}</div>`
        )
        .join("");
    }
    html += `</body></html>`;
          ...
          {nav === "about" && (
            <div style={styles.container}>
              ...
              <div style={{ marginTop: 24, color: "#f7f7fa", lineHeight: 1.8 }}>
                <p>
                  VulnX is an automated website vulnerability scanner designed to help security professionals and developers identify common security weaknesses in web applications.
                </p>
                ...
                <p style={{ color: "#ff7a7a" }}>
                  This tool is for educational and authorized testing purposes only. Always obtain proper permission before scanning any website you do not own.
                </p>
              </div>
            </div>
          )}
              ...
              {!acceptedTerms ? (
                <motion.div key="modal" variants={pageVariants} initial="hidden" animate="enter" exit="exit" style={modalStyles.overlay}>
                  <div style={modalStyles.modal}>
                    ...
                    <p>
                      <b>Important:</b> By proceeding, you confirm that you own or have explicit permission to scan the target website for vulnerabilities.<br />
                      <br />
                      Unauthorized security testing of systems you do not own or have written permission for may violate the law, terms of service, or ethical standards.<br />
                      <br />
                    ...
                    <button style={{ ...styles.button, marginTop: 12, background: "#30313a" }} onClick={() => setNav("home")}>
                      Go Back
                    </button>
                  </div>
                </motion.div>
                ...
                <motion.div key="scanForm" variants={pageVariants} initial="hidden" animate="enter" exit="exit" style={styles.container}>
                  ...
                  <form onSubmit={handleScan} style={styles.form}>
                    <input
                      type="text"
                      placeholder="Enter website URL (e.g., https://test-vuln.com)"
                      value={url}
                  ...
                  {result && (
                    <div style={styles.results}>
                      <h2>Scan Results</h2>
                      {result.length === 0 ? (
                        <p>No vulnerabilities found. Site looks secure!</p>
                      ) : (
                        result.map((vuln, index) => (
                          <div key={index} style={styles.vulnerability}>
                            <strong>{vuln.type}</strong> at <code>{vuln.url}</code>
                            {vuln.risk && <span style={{ float: 'right', color: '#ff7a7a' }}>[{vuln.risk}]</span>}
                            {vuln.description && <div style={{fontSize: '90%', opacity: 0.8, marginTop: '4px'}}>{vuln.description}</div>}
                          </div>
                        ))
                      )}
